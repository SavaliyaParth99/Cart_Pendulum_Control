...
#define Ref_Move_C 5
#define Ref_Done 6
#define L_Ref_Speed -180
#define R_Ref_Speed  180
//static volatile short int L_Ref_Speed;
//static volatile short int R_Ref_Speed;
static volatile short int Cart_Ref_Pos;
static volatile unsigned int Cart_Pos;
static volatile int Cart_Pos_Last;
static volatile unsigned int Pend_Ang;
static volatile short int Pend_Ang_Last;
static volatile unsigned int Start_Control;
static volatile int Control_Mode;
static volatile int Force;

#define Cntrl_Init 0
#define Cntrl_Start 1
#define Cntrl_Stop 2
...
static volatile int Cart_Pos_Gui;
static volatile int Cart_Pos_Last_Gui;
static volatile int Cart_Velo_Gui;
static volatile short int Pend_Ang_Gui;
static volatile short int Pend_Ang_Last_Gui;
static volatile short int Pend_AngularVelocity_Gui;
static volatile short int Cart_Ref_Pos_Gui;
static volatile short int Angle_e;
...
void Inverter_Off()
{
   ...
   /*
   * Sends Force, Cart_Position, Cart_Velocity, Pend_Ang, Pend_AngularVelocity to GUI for
   display
   */
static void sendDataGUI()
{
	Cart_Pos_Last_Gui = Cart_Pos_Gui;
	Pend_Ang_Last_Gui = Pend_Ang_Gui;
	// IENC1_REG write => latch counters
	IENC1_REG = 0;
	Cart_Pos = IENC0_REG;
	// Check if negative, 19th bit is signed
	if ((Cart_Pos & 0x080000) != 0)
	{
		Cart_Pos |= 0xFFF00000;
	}
	// Convert to IEEE standard
	Cart_Pos_Gui = *(int *)&(Cart_Pos);
	Pend_Ang = IENC1_REG;
	// Check if negative, 19th bit is signed
	if ((Pend_Ang & 0x080000) != 0)
	{
		Pend_Ang |= 0xFFF00000;
	}
	// Convert to IEEE standard
	Pend_Ang_Last = *(short *)&(Pend_Ang);
	// Check if negative, 19th bit is signed
	if ((Angle & 0x080000) != 0)
	{
		Angle |= 0xFFF00000;
	...
	if (Pend_Ang_Last > 0)
	{
		Pend_Ang_Last = Pend_Ang_Last - Angle_e;
	}
	else
	{
		Pend_Ang_Last = Pend_Ang_Last + Angle_e;
	}
	Pend_Ang_Gui = (-1) * Pend_Ang_Last;
	Cart_Velo_Gui = (Cart_Pos_Gui - Cart_Pos_Last_Gui);
	Pend_AngularVelocity_Gui = (Pend_Ang_Gui - Pend_Ang_Last_Gui);
	Force_Gui = Force;
	Cart_Ref_Pos_Gui = (Cart_Ref_Pos);
	if (Record)
	{
		// For logging data start string with "~"
	printf("~ %d %d %d %d %d %d\n", Cart_Ref_Pos_Gui, Cart_Pos_Gui, Cart_Velo_Gui, Pend_Ang_Gui, Pend_AngularVelocity_Gui, Force_Gui);
	printf("~ %d %d %d %d %d %d\n", Cart_Ref_Pos_Gui, Cart_Pos_Gui, Cart_Velo_Gui, Pend_Ang_Gui, Pend_AngularVelocity_Gui, Force_Gui);
	}
...
static void TimerIntrHandler(void *CallBackRef)
{
    ...
    if (Start_Ref)
    {
        ...
        case Ref_Move_L:
			...
			// Convert to IEEE standard
			Cart_Pos_Last = *(int *)&(Cart_Pos);
//			printf(" ***RMLCart_Pos_Last: %10d \n\r", Cart_Pos_Last);
            Force = Fixedpt_Speed_Control(L_Ref_Speed, Cart_Pos_Last);
//            printf(" ***RMLforce: %10d \n\r", Force);
            DacSet(0, Force);
//            DacSet(0, -120);
        ...
        case Ref_Move_R:
			...
			// Convert to IEEE standard
			Cart_Pos_Last = *(int *)&(Cart_Pos);
//			printf(" ***RMRCart_Pos_Last: %10d \n\r", Cart_Pos_Last);
            Force = Fixedpt_Speed_Control(R_Ref_Speed, Cart_Pos_Last);
//            printf(" ***RMRforce: %10d \n\r", Force);
            DacSet(0,Force);
//			DacSet(0, 120);
            IENC1_REG = 0;  // IENC1_REG write => latch counters
        ...
        case Ref_Pause_R:
            ...
            R_End_Pos_Last = *(int *)&(R_End_Pos);
//            printf(" ***IENC0_REG PauseRightAftercalc: %7d \n\r", R_End_Pos_Last);
            Mid_Pos = R_End_Pos_Last / 2;
            //printf(" ***Mid_Pos : %7d \n\r",Mid_Pos);
            //IENC0_REG = 0;
            Ref_Mode = Ref_Move_C;
            break;
...
//            DacSet(0, -120);
            ...
            Check_Cart_Pos_Last = *(int *)&(Check_Cart_Pos);

            //printf(" ** Check_Cart_Pos_Last MoveCenter: %7d \n\r",Check_Cart_Pos_Last);
            if (Check_Cart_Pos_Last < (Mid_Pos))
            //if (Check_Cart_Pos_Last == Mid_Pos)
            {
    ...
    else if(Start_Control)
	{
		if ((LEFT_SWITCH) || (RIGHT_SWITCH))
		{
			...
			DacSet(0, 0);
			Inverter_Off();
			printf("Control Pase cannot start as Left or Right Switch is active.\n\r");
		}
		else if (!AMPL_OK)
		{
			printf("inverter not OK\n\r");
		...
		else if (Done_Ref)
		{
			...
			case Cntrl_Init:
				...
				Pend_Ang = 0;
				Pend_Ang_Last = 0;
				Control_Mode = Cntrl_Start;
				break;
			case Cntrl_Start:
				if (!Start_Control)
				{
					Control_Mode = Cntrl_Stop;
					break;
				}
				// IENC1_REG write => latch counters
				IENC1_REG = 0;
				...
				// Convert to IEEE standard
				Pend_Ang_Last = -(*(short *)&(Pend_Ang));
				// printf("pend_ang_Last %d %d \n\r",Pend_Ang,Pend_Ang_Last);

				Force = Fixedpt_Position_Control(Cart_Ref_Pos, Pend_Ang_Last, Cart_Pos_Last);
				// printf("Force : %d \n",Force);
				DacSet(0, Force);
				break;
			case Cntrl_Stop:
...
int main()
{
    ...
    do
    {
        ...
        if (!strncmp(cbuf, "cntrl", 5))
		{

			printf("cart_ref_pos: (main): %d", Cart_Ref_Pos);
			if (Cart_Ref_Pos != 1)
			{
				printf("cart_ref_pos: %d", Cart_Ref_Pos);
				printf(" Reference cart position I/p error\n\r");
			}
			else
			{
				if (!AMPL_OK)
