...
s16 Pos_Cntrl_FixedPt (s32 Ref_Pos, s16 Pend_Angle, s32 Cart_Position)
{
    ...
    s16 Kst_f[]={-19348,-25061,17564,363};
    
    /*PI Controller logic*/
    Control_Err = (Ref_Pos - Cart_Position);
    Gi_mul = Control_Err*Gi_i;
    if (Gi_mul > upLI_f){
        Gi_mul = upLI_f;
    }
    else if(Gi_mul< dnLI_f){
        Gi_mul = dnLI_f;
    }
    Cur_i_state = Cur_i_state + Gi_mul;
    Gp_mul = (Control_Err*Gp_i)<<7;
