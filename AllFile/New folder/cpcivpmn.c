...
static void TimerIntrHandler(void *CallBackRef)
{
	...
	if (Start_Referencing && !Start_Control)
	{
		...
		case RP_Move_Left:
			...
			// Force = Speed_Ctrl_FixedPt(Ref_Speed_Left, Cart_Position_i,Pend_Angle_i);
		...
		case RP_Move_Right:
			...
			// Force = Speed_Ctrl_FixedPt(Ref_Speed_Right, Cart_Position_i,Pend_Angle_i);
			// Set DAC with force for controlling cart speed
			DacSet(0, 149);
			// IENC1_REG write => latch counters
			IENC1_REG = 0;
	...
	else if (Start_Control)
	{
		...
		else if (Done_Referencing)
		{
			...
			case CP_Start:
				...
				Force = Pos_Cntrl_FixedPt(Cart_Ref_Position, Cart_Position_i, Pend_Angle_i);
