...
#define Ref_Speed_L -4915
#define Ref_Speed_R 4915

static volatile short int Cart_Ref_Pos;
static volatile unsigned int Cart_Pos;
static volatile int Cart_Pos_e;
static volatile unsigned int Pend_Ang;
static volatile short int Pend_Ang_e;
...
static volatile int Cart_Pos_Prev_Gui;
static volatile int Cart_Velo_Gui;
static volatile short int Pend_Ang_Gui;
static volatile short int Pend_Ange_Prev_Gui;
...
static void TimerIntrHandler(void *CallBackRef)
{
    ...
    if (Start_Ref)
    {
        ...
        case Ref_Move_L:
            ...
            Cart_Pos_e = *(int *)&(Cart_Pos);
            Force = Fixedpt_Speed_Control(Ref_Speed_L, Cart_Pos_e,Pend_Ang_e);
            //DacSet(0, -100); 
            DacSet(0, Force);
    ...
    else if (Start_Control)
	{
		...
		else if (Done_Ref)
		{
			...
			case CP_On:
				if (!Start_Control)
				{
					// Go to CP_Off stop controller requested
					Control_Mode = CP_Off;
					break;
				}
				...
				// Convert to IEEE standard
				Pend_Ang_e = (*(short *)&(Pend_Ang)) * (-1);
				// printf("pend_ang_i %d %d \n\r",Pend_Angle,Pend_Angle_i);
				// Get force value from cart position controller
				Force = Fixedpt_Position_Control(Cart_Ref_Pos, Cart_Pos_e, Pend_Ang_e);
...
int main()
{
    ...
    do
    {
        ...
        else if (!strncmp(cbuf, "cntrl", 4))
		{

			printf("cart_ref_pos: (main): %d", Cart_Ref_Pos);
			if (Cart_Ref_Pos != 1)
			{
				printf("cart_ref_pos: (loop): %d", Cart_Ref_Pos);
